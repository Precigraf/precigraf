...
import { Search, Plus, Star, Trash2, Package } from 'lucide-react';
...
export const CatalogProductsManager: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      <CatalogCategoryChips selectedId={filterCat} onSelect={setFilterCat} />
            ...
            {filtered.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      update.mutate({ id: p.id, is_featured: !p.is_featured })
                    }
                    className="p-1.5 hover:bg-muted rounded-md"
                  >
                    <Star
                      className={`w-4 h-4 ${
                        p.is_featured ? 'fill-amber-400 text-amber-400' : 'text-muted-foreground'
                      }`}
                    />
                  </button>
                  <button
