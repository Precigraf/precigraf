...
const MarketplaceImpact: React.FC<MarketplaceImpactProps> = ({
  ...
  const suggestedMargin = feesExceedingProfit ? Math.ceil(profitImpactPercentage + 30) : null;
        ...
        <div className="w-full bg-primary/10 border border-primary/30 rounded-xl p-4 mt-2 flex flex-col gap-4">
          ...
          {onApplySuggestedMargin && (
            <div className="w-full">
              ...
              >
                {applied ? (
                  <><Check className="w-4 h-4 mr-2 flex-shrink-0 text-success" /><span>Margem aplicada!</span></>
                ) : (
                  <><Lightbulb className="w-4 h-4 mr-2 flex-shrink-0" /><span>Aplicar margem de {suggestedMargin}%</span></>
                )}
              </Button>
            </div>
          )}
