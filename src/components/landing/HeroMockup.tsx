...
const Row: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div className="flex items-center justify-between gap-3 text-xs sm:text-sm py-2 border-b border-border/60 last:border-0 min-w-0">
    ...
    <div className="relative w-full min-w-0">
      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-5 gap-4 min-w-0">
        ...
        <Card className="lg:col-span-3 min-w-0 p-5 sm:p-6 bg-card border-border shadow-card rounded-2xl">
          ...
          <div className="mt-5 flex items-center justify-between gap-3 p-3 rounded-lg bg-muted/60 min-w-0">
            <span className="text-xs sm:text-sm font-medium text-foreground truncate">Custo total</span>
        ...
        {/* Result card */}
        <Card
          className="@container relative lg:col-span-2 min-w-0 w-full p-6 sm:p-7 bg-foreground text-background border-foreground rounded-2xl shadow-[0_10px_40px_-12px_hsl(var(--foreground)/0.35)] flex flex-col gap-5 lg:min-h-[380px] overflow-hidden"
        >
          ...
          <div className="relative min-w-0">
            ...
            <p className="font-bold tabular-nums leading-[1.05] mt-2 tracking-tight whitespace-nowrap text-[clamp(1.5rem,7.5cqw,2.25rem)] pr-1">
              {fmtBRL(FINAL_PRICE)}
            </p>
          ...
          <div className="relative grid grid-cols-2 gap-3 auto-rows-fr">
            {[
              ...
              <div
                key={label}
                className="flex flex-col justify-between min-w-0 rounded-xl bg-background/10 border border-background/15 px-3 py-3 hover:bg-background/15 transition-colors"
              >
                <div className="flex items-center gap-1.5 text-[10px] text-background/70 min-w-0">
                ...
                <p className="font-bold mt-2 tabular-nums leading-none whitespace-nowrap text-[clamp(0.85rem,4.8cqw,1.05rem)]">
                  {value}
                </p>
              </div>
            ))}
