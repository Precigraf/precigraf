  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('profiles')
        .select('store_name, system_color, logo_url, logo_scale, company_name, company_document, company_phone, company_email, company_address, company_address_number, company_neighborhood, company_city, company_state, company_cep, pix_key, whatsapp, company_full_address')
        .eq('user_id', user!.id)
        .single();
