...
ALTER TABLE public.catalog_product_variants
  ...
  ADD COLUMN IF NOT EXISTS stock_type text NOT NULL DEFAULT 'infinite';

ALTER TABLE public.catalog_product_variants
  DROP CONSTRAINT IF EXISTS catalog_product_variants_stock_type_check;
ALTER TABLE public.catalog_product_variants
  ADD CONSTRAINT catalog_product_variants_stock_type_check
  CHECK (stock_type IN ('infinite','limited'));
      ...
      SELECT json_agg(json_build_object(
        ...
        'variants', COALESCE((
          SELECT json_agg(json_build_object(
            ...
            'promo_price', v.promo_price, 'stock', v.stock,
            'stock_type', v.stock_type, 'is_active', v.is_active
